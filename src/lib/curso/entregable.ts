// ─────────────────────────────────────────────────────────────────────────────
// CURSO · DOCUMENTO DE TRABAJO
//
// Una sola función produce el trabajo completo, y ese objeto alimenta tanto la
// vista final como la descarga. Así ninguna de las dos puede mostrar una
// versión ligeramente distinta del mismo trabajo.
//
// Módulo puro: sin APIs de navegador, para poder probarlo sin montar nada. El
// documento se descarga; no se envía a ninguna parte, porque no hay ninguna
// parte a la que enviarlo.
// ─────────────────────────────────────────────────────────────────────────────
import { blameOptions, claimActions } from '@/content/curso/actividades';
import { summarizeDraft, type DraftSummary } from '@/content/curso/lab';
import { CURSO_META, GUIDING_QUESTION } from '@/content/curso/etapas';
import { AI_TOOLS } from '@/content/curso/prompts';
import type { CursoState } from './estado';
...
export interface CursoEntregable {
  meta: {
    courseTitle: string;
    generatedAt: string;
  };
...
function answerOf(q: CursoState['initialQuestion']): SubmissionAnswer {
  return {
    answer: q.blame,
...
/**
 * Construye el documento de trabajo. `generatedAt` se inyecta para que la
 * función sea determinista y comprobable; por defecto usa el reloj del entorno.
 */
export function buildEntregable(
  state: CursoState,
  ...
  return {
    meta: {
      courseTitle: CURSO_META.title,
      generatedAt,
    },
    identity: { name: state.identity.name.trim() },
...
/** Documento único del curso, en Markdown legible en cualquier editor. */
export function renderEntregableMarkdown(s: CursoEntregable): string {
  const out: string[] = [];

  out.push(`# ${s.meta.courseTitle}`);
  out.push('');
  out.push(`**Autor** · ${value(s.identity.name)}`);
...
/** `construye-tu-prompt_ana-perez.md` */
export function entregableFilename(s: CursoEntregable): string {
  const slug =
    s.identity.name
      .normalize('NFD')
  ...
  return `construye-tu-prompt_${slug}.md`;
}
