...
export default function PromptLabPage() {
  ...
  const downloadTxt      = () => downloadBlob(promptResult, 'prompt-juridico.txt');
  const downloadMd       = () => downloadBlob(`# Prompt jurídico\n\n\`\`\`\n${promptResult}\n\`\`\`\n`, 'prompt-juridico.md', 'text/markdown;charset=utf-8');
  const downloadSys      = () => sel.modelo && downloadBlob(generateSystemPrompt(promptResult, sel.modelo), 'system-prompt.txt');
  const downloadGPT      = () => downloadBlob(generateGPTInstructions(promptResult), 'gpt-instructions.txt');
  const downloadClaude   = () => downloadBlob(generateClaudeProject(promptResult), 'claude-project.txt');
